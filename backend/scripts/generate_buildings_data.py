...
OUTPUT_JSON_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', 'data', 'buildings_seed.json'))
...
def generate_data():
    if not os.path.exists(CITY_JSON_PATH):
        print(f"File not found: {CITY_JSON_PATH}")
        return
    ...
    # Shuffle cities to get random ones
    ...
    for city in cities:
        ...
        # Generate 1-2 buildings per city to reach 20 faster if needed, 
        # but let's try to spread across cities.
        num_buildings_in_city = random.randint(1, 2)
        for _ in range(num_buildings_in_city):
            if count >= 20:
                break
            ...
            building_code = f"BLD-{random.randint(1000, 9999)}"
            
            building_data = {
                "building_code": building_code,
                "building_name": building_name,
                "description": f"Tòa nhà hiện đại tại {district['name']}, {city['name']}. Đầy đủ tiện nghi, an ninh 24/7.",
                "status": "ACTIVE",
                "address": {
                    "address_line": f"{random.randint(1, 500)} Đường {random_name}",
                    "ward": district['name'],
                    "city": city['name'],
                    "country": "Vietnam"
                }
        ...
        # Sleep a bit to avoid hitting API too hard
        time.sleep(0.5)

    with open(OUTPUT_JSON_PATH, 'w', encoding='utf-8') as f:
        json.dump(buildings, f, ensure_ascii=False, indent=4)
    
    print(f"\nSuccessfully generated {len(buildings)} buildings and saved to {OUTPUT_JSON_PATH}")
